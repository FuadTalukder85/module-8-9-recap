...
const userNameSchema = new Schema<UserName>({
  firstName: {
    type: String, required: [true, 'First name is required.'], trim: true, maxlength: [20, 'first name can not be more than 20 characters'],
    // validate: {
...
const studentSchema = new Schema<Student>({
  ...
  name: { type: userNameSchema, required: [true, 'Student name details are required.'] },
  gender: {
    type: String,
    enum: { values: ['male', 'female', 'other'], message: 'Invalid gender selection.' },
